...
### 38. Live-stack access is the orchestrator's; agents work from code and never hang on a command

Two failure modes, one rule. Observed running a live-Docker project: an `architect` — a doc-only task — ran `docker compose run app …` to inspect the live DB for its design doc; the command hung for **19 minutes** on a connection that never returned and blocked the whole agent. Across the session, multiple subagents reached for `docker compose run` against the live stack, and every such call was slow.

**Agents work from code; the orchestrator owns the live stack.** Non-verification agents (`architect`, `spec-writer`, `planner`, `doc-keeper`, `test-author`) have no reason to query or mutate running services — they work from **source** (`Read`/`Grep`), which is fast and cannot hang on a socket. Even `implementer`/`adversary` do not drive live *mutation* (it is permission-gated anyway) and do not inspect the live stack for design or review. **All live-stack inspection and verification belong to the orchestrator** — the explicit live-verify step of the build loop (the `night-build` skill), not scattered across subagents.

**No command may hang an agent.** Any potentially-blocking or stack-touching shell command MUST pass an explicit wall-clock `timeout` (e.g. `timeout 60 …`) so a hung service, a never-returning query, or an interactive prompt returns control instead of blocking the agent's tool loop indefinitely. This is the *prevention* companion to [#24](#24-orchestrator-proactively-checks-in-on-long-running-background-agents), which *detects* a stuck agent after the fact.

**Project-scaffold note (advisory).** A tools-container that bind-mounts the working dir can pay a per-command tax — e.g. every `uv run` rebuilding a mounted host `.venv` (~30s) because the host interpreter is not present in the container. Exclude the venv from the mount (an anonymous volume over the venv path, or a container-internal venv). The template ships no compose file, so this is guidance for repos that add one.

**Failure mode this prevents.** Operator perception was "why is everything so slow / is the stack broken?" — the stack was healthy; the slowness was **agents driving the live stack** (hanging on it, and paying the mount tax on every call).
